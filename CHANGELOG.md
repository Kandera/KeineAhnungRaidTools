**English** | [Deutsch](https://github.com/Kandera/KeineAhnungRaidTools/blob/main/CHANGELOG-de.md)
...
- **Two copies of one item now share one vote window**, and your answer counts for both.
- **KART warns about items the council has not decided yet** before their trade time runs out.
...
- **The warning about undecided items keeps running** after the last trade is done, and after a reload.
- **An item whose trade window ran out says so** instead of claiming it could never have been handed over.
...
- **A boss no longer floods the raid with one message per item** — everything that drops together is announced together.
- **The lootmaster is quieter about what is on the table**, and a change reaches the raid faster than before.
- **Less addon chatter during loot**, so announcements get through faster when the raid is busy.
- **Two versions of the same item are told apart again**, so handing one over no longer ticks the other off what you still owe.
- **Only one person hands out the loot**, even right after a reload — the raid lead no longer takes over from a lootmaster who is still there.
- **A vote now repeats itself until the council has it.**
...
- **An award announced while your client was still starting up reaches your loot history after all.**
- **Whoever hands out the loot is told when an item went to somebody else after all**, instead of promising an item they do not have.
...
- **The two relevance switches are greyed out for now.** Hiding irrelevant items and the automatic transmog vote stay off until the known defects in them are fixed.
