...
-- Guard rail for the durable notice queue (see PersistNotice), not a working limit: the queue holds
-- one line per pending trade that crosses a line during a pull, realistically one to three a night.
-- 20 is there for the case nobody foresaw.
local NOTICE_QUEUE_CAP = 20
-- How long a restored notice waits before printing at load (see Trade.RestorePersistedTrades), so it
-- does not vanish into the rest of the login output.
local NOTICE_PRINT_DELAY = 5
...
combatFrame:SetScript("OnEvent", function(_, event)
    ...
    -- Said in full above, so the durable copy (see PersistNotice) is not needed any more either --
    -- otherwise it would print again after a reload that happens to land after this pull.
    if KART_LCTrades and KART_LCTrades.notices then wipe(KART_LCTrades.notices) end
end)

-- Durable copy of a notice queued while in combat, for the two removal notices only (see
-- TradeNotice's persist flag) -- the ones with no onSaid and nothing left to regenerate them, so
-- deferredNotices being memory-only would otherwise lose them to a reload or a disconnect mid-pull.
...
--
-- persist (optional) additionally writes the queued line into KART_LCTrades (see PersistNotice), for
-- the two notices whose entry was just REMOVED -- the trade window running out and the item turning
-- out to be untradeable. Those two have no onSaid and nothing regenerates them, unlike the two
-- warnings: LC_TRADE_TIMEOUT_WARNING is regenerated by the five-minute ticker from LC.pendingTrades
-- itself (which IS persisted), and a restored copy could not carry onSaid's latch anyway, so it would
-- queue again every five minutes. Leave it unset for every other caller.
local function TradeNotice(text, onSaid, key, persist)
...
function Trade.CheckTradeTimeouts()
    ...
    for i = #LC.pendingTrades, 1, -1 do
        ...
        if remaining == 0 and stampElapsed < TRADE_TIMEOUT_SECONDS then
            ...
            dropped = true
            -- persist = true: the entry is gone and nothing regenerates this line (see TradeNotice).
...
function Trade.RestorePersistedTrades()
    ...
    -- Same convention as pruneExpired above: a queued line describes the same four-hour clock, so
    -- once TRADE_TIMEOUT_SECONDS has passed the explanation it carries is moot and dropped rather
    -- than printed.
    for i = #store.notices, 1, -1 do
        local e = store.notices[i]
        if type(e) ~= "table" or type(e.text) ~= "string"
           or type(e.at) ~= "number" or (now - e.at) >= TRADE_TIMEOUT_SECONDS then
    ...
    -- The two removal notices that survived the reload (see PersistNotice). A reload can land
    -- mid-pull: still in combat here means the drain that would print these hasn't happened yet, so
...
function Trade.ClearRollState(rollID)
    ...
    -- true here and must not be assumed anywhere. LC.SnapshotRollInstance keeps an existing NAMED raid
    -- whenever the ITEM in the row matches -- wherever the client is standing when the second read
    -- happens, since B153 (a read from inside an instance says where the CLIENT is, and this field
    -- records where the ITEM dropped); only the two manual doors declare a new drop and overwrite
    -- regardless. So a rollID Blizzard reuses for the same item can carry the previous roll's raid on
    -- any client, which is accepted and written down (B150, B151, B152 in docs/BACKLOG.md, B153 in the
    -- commit history) -- read that note before building anything on this clear.
    Trade.PruneExpiredLootStamps()
